...
Modified on Tue Jun 27 16:58:52 2017
...
class Net(object):
	...
	def conv2d(self, scope, input, kernel_size, stride = 1, pretrain = True, train = True):
		...
		with tf.variable_scope(scope) as scope:
			kernel = self._variable_with_weight_decay('weights', shape = kernel_size, stddev = 5e-2, wd = self.weight_decay, pretrain = pretrain, train = train)
	...
	def local(self, scope, input, in_dimension, out_dimension, leaky=True, pretrain=True, train=True):
		...
		with tf.variable_scope(scope) as scope:
			...
			weights = self._variable_with_weight_decay('weights', shape = [in_dimension, out_dimension], stddev = 0.04, wd = self.weight_decay, pretrain = pretrain, train = train)
