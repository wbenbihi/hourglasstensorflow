# -*- coding: utf-8 -*-
"""
Original Author : nilboy
GitHub repo : https://github.com/nilboy/tensorflow-yolo

Modified on Sat Jun 24 16:59:40 2017
Modification: Python 3.5 compatibility
As part of https://github.com/wbenbihi/hourglasstensorlfow
"""
...
class YoloTinyNet(Net):
	...
	def inference(self, images):
		...
		temp_pool = self.max_pool(temp_conv, [2,2], 2)
		temp_conv = self.conv2d('conv' + str(conv_num), temp_pool, [3,3,64,128], stride = 1)		
		conv_num += 1
		
		temp_pool = self.max_pool(temp_conv, [2,2], 2)
		temp_conv = self.conv2d('conv' + str(conv_num), temp_pool, [3,3,128,256], stride = 1)		
		conv_num += 1
		
		temp_pool = self.max_pool(temp_conv, [2,2], 2)
		temp_conv = self.conv2d('conv' + str(conv_num), temp_pool, [3,3,256,512], stride = 1)		
		conv_num += 1
		
		temp_pool = self.max_pool(temp_conv, [2,2], 2)
		temp_conv = self.conv2d('conv' + str(conv_num), temp_pool, [3,3,512,1024], stride = 1)		
		conv_num += 1
		...
		local3 = tf.concat([class_probs, scales, boxes], 3, name= 'predicts')
		predicts = local3
	...
	def body1(self, num, object_num, loss, predict, labels, nilboy):
		...
		#coord loss
		coord_loss = (tf.nn.l2_loss(I * (p_x - x)/(self.image_size / self.cell_size)) + tf.nn.l2_loss(I * (p_y - y)/(self.image_size / self.cell_size)) + tf.nn.l2_loss(I * (p_sqrt_w - sqrt_w)/(self.image_size )) + tf.nn.l2_loss(I * (p_sqrt_h - sqrt_h)/(self.image_size ))) * self.coord_scale
		nilboy = I
